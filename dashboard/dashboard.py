...
# ======================
df = orders_filtered.merge(customers, on="customer_id", how="left")
...
# TITLE
# ======================
st.title("📊 E-Commerce Dashboard (Optimized Version)")
...
top_seller = df.groupby('seller_id')['price'].sum().sort_values(ascending=False).head(10)
...
top_category = df.groupby('product_category_name_english')['price'].sum().sort_values(ascending=False).head(10)
...
st.write("""
- Payment method tertentu paling dominan  
- Top seller menyumbang revenue terbesar  
- Produk tertentu sangat laris  
- Review mayoritas tinggi  
""")
...
2. Pertahankan top seller  
3. Evaluasi produk rating rendah  
4. Optimalkan kategori terlaris  
""")
